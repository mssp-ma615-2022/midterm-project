...
strawb2 %<>% mutate(organic=ifelse(str_detect(strawb2$temp1, "ORGANIC"),1,0), .after=temp1)
...
strawb_organic <- strawb %>% slice(org_row, preserve = FALSE)
strawb_non_organic <- strawb %>% filter(!row_number() %in% org_row)

# grep chemical rows 
chem_rows <- grep("APPLICATIONS", 
                  strawb_non_organic$sales, 
                  ignore.case = T)
...
# There is only missing chemical code
